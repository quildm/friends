...
class UserManager(models.Manager):
    def register(self, name, alias, email, password, password_confirmation, birthday):
    	print "in the UserManager register function"
        errors = {}
        if (len(name) < 2):
        ...
        if (errors):
			print "finished the UserManager register function with errors"
			return (False, errors)
        else:
            password = bcrypt.hashpw(password.encode('utf-8'), bcrypt.gensalt())

            self.create(email=email, name=name, alias=alias, password=password, birthday=birthday)
            print "finished the UserManager register function with no errors"
            return (True, self.get(email=email))

    def login(self, email, password):
    	print "in the UserManager's login function"
        try:
